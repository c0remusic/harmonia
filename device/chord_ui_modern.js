...
function drawSyncButton(g, r) {
	...
	// Quand pressé: inverser les couleurs (fond = doré, texte = sombre) peu importe où est la souris
...
function drawVLModeButton(g, r, mode, isHover, pressTime) {
	...
	// Texte: gris clair pour ANCHOR (comme VOICE LEADING OFF), dark pour RELATIVE et PIANO
	var lbl = (mode==="anchored")?"ANCHOR":(mode==="relative")?"RELATIVE":"PIANO";
	var textColor = (mode === "anchored") ? [0.72, 0.72, 0.75] : COLORS.text_dark;
	g.set_source_rgba(textColor[0], textColor[1], textColor[2], 1.0);
	g.set_font_size(9);
...
// relâchement souris → note-off (jeu momentané)
function ondrag(x, y, but) {
	if (but === 0 && activeCol >= 0) {
		activeCol = -1; activeRow = -1;
		outlet(0, "release");
		mgraphics.redraw();
	}
